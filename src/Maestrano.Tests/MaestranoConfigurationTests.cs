using Microsoft.VisualStudio.TestTools.UnitTesting;
using System;

namespace Maestrano.Tests
{
    [TestClass]
    public class MaestranoConfigurationTests
    {
        [TestMethod]
        public void itHasTheRightDefaultTestConfig()
        {
            Mno.Environment = "test";

            // App
            Assert.AreEqual("http://localhost", Mno.App.Host);

            // API
            Assert.AreEqual("http://api-sandbox.maestrano.io", Mno.Api.Host);
            Assert.AreEqual("/api/v1/", Mno.Api.Base);
            Assert.AreEqual("C#", Mno.Api.Lang);
            Assert.AreEqual(Mno.Version, Mno.Api.Version);
            Assert.AreEqual(Environment.OSVersion.ToString() + " - " + Environment.Version.ToString(), Mno.Api.LangVersion);

            // Webhook
            Assert.AreEqual("/maestrano/account/groups/:id", Mno.Webhook.Account.GroupsPath);
            Assert.AreEqual("/maestrano/account/groups/:group_id/users/:id", Mno.Webhook.Account.GroupUsersPath);

            // SSO
            Assert.IsTrue(Mno.Sso.Enabled);
            Assert.IsTrue(Mno.Sso.SloEnabled);
            Assert.AreEqual("http://api-sandbox.maestrano.io", Mno.Sso.Idp);
            Assert.AreEqual("http://localhost", Mno.Sso.Idm);
            Assert.AreEqual("virtual", Mno.Sso.CreationMode);
            Assert.AreEqual("/maestrano/auth/saml/init.aspx", Mno.Sso.InitPath);
            Assert.AreEqual("/maestrano/auth/saml/consume.aspx", Mno.Sso.ConsumePath);
            Assert.AreEqual("urn:oasis:names:tc:SAML:2.0:nameid-format:persistent", Mno.Sso.NameIdFormat);
            Assert.AreEqual("01:06:15:89:25:7d:78:12:28:a6:69:c7:de:63:ed:74:21:f9:f5:36", Mno.Sso.X509Fingerprint);
            Assert.AreEqual("-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----", Mno.Sso.X509Certificate);
        }

        [TestMethod]
        public void itHasTheRightDefaultProductionConfig()
        {
            Mno.Environment = "production";

            // App
            Assert.AreEqual("http://localhost", Mno.App.Host);

            // API
            Assert.AreEqual("https://maestrano.com", Mno.Api.Host);
            Assert.AreEqual("/api/v1/", Mno.Api.Base);
            Assert.AreEqual("C#", Mno.Api.Lang);
            Assert.AreEqual(Mno.Version, Mno.Api.Version);
            Assert.AreEqual(Environment.OSVersion.ToString() + " - " + Environment.Version.ToString(), Mno.Api.LangVersion);

            // Webhook
            Assert.AreEqual("/maestrano/account/groups/:id", Mno.Webhook.Account.GroupsPath);
            Assert.AreEqual("/maestrano/account/groups/:group_id/users/:id", Mno.Webhook.Account.GroupUsersPath);

            // SSO
            Assert.IsTrue(Mno.Sso.Enabled);
            Assert.IsTrue(Mno.Sso.SloEnabled);
            Assert.AreEqual("https://maestrano.com", Mno.Sso.Idp);
            Assert.AreEqual("http://localhost", Mno.Sso.Idm);
            Assert.AreEqual("virtual", Mno.Sso.CreationMode);
            Assert.AreEqual("/maestrano/auth/saml/init.aspx", Mno.Sso.InitPath);
            Assert.AreEqual("/maestrano/auth/saml/consume.aspx", Mno.Sso.ConsumePath);
            Assert.AreEqual("urn:oasis:names:tc:SAML:2.0:nameid-format:persistent", Mno.Sso.NameIdFormat);
            Assert.AreEqual("2f:57:71:e4:40:19:57:37:a6:2c:f0:c5:82:52:2f:2e:41:b7:9d:7e", Mno.Sso.X509Fingerprint);
            Assert.AreEqual("-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----", Mno.Sso.X509Certificate);
        }

        [TestMethod]
        public void itSetsTheHostAndIdmProperlyIfDefined()
        {
            Mno.Environment = "production";

            string expected = "https://mysuperapp.com";
            Mno.App.Host = expected;

            Assert.AreEqual(expected, Mno.App.Host);
            Assert.AreEqual(expected, Mno.Sso.Idm);
        }

        [TestMethod]
        public void itSetsTheApiTokenProperly()
        {
            Mno.Environment = "production";
            Mno.Api.Id = "app-1";
            Mno.Api.Key = "bla";

            Assert.AreEqual("app-1:bla", Mno.Api.Token);
        }
    }
}
