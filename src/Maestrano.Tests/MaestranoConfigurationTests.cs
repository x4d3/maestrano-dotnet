...
namespace Maestrano.Tests
{
    ...
    public class MaestranoConfigurationTests
    {
        ...
        public void itHasTheRightDefaultTestConfig()
        {
            ...
            // API
            Assert.AreEqual("http://api-sandbox.maestrano.io", MnoHelper.Api.Host);
            ...
            Assert.AreEqual("http://api-sandbox.maestrano.io", MnoHelper.Sso.Idp);
            Assert.AreEqual("http://localhost", MnoHelper.Sso.Idm);
            Assert.AreEqual("virtual", MnoHelper.Sso.CreationMode);
            Assert.AreEqual("/maestrano/auth/saml/init.aspx", MnoHelper.Sso.InitPath);
            Assert.AreEqual("/maestrano/auth/saml/consume.aspx", MnoHelper.Sso.ConsumePath);
            Assert.AreEqual("urn:oasis:names:tc:SAML:2.0:nameid-format:persistent", MnoHelper.Sso.NameIdFormat);
            Assert.AreEqual("01:06:15:89:25:7d:78:12:28:a6:69:c7:de:63:ed:74:21:f9:f5:36", MnoHelper.Sso.X509Fingerprint);
            ...
            // Connec
            Assert.AreEqual("http://api-sandbox.maestrano.io", MnoHelper.Connec.Host);
            Assert.AreEqual("/connec/api/v2", MnoHelper.Connec.BasePath);
        ...
        public void itHasTheRightDefaultProductionConfig()
        {
            ...
            // API
            Assert.AreEqual("https://maestrano.com", MnoHelper.Api.Host);
            Assert.AreEqual("/api/v1/", MnoHelper.Api.Base);
            Assert.AreEqual("C#", MnoHelper.Api.Lang);
            Assert.AreEqual(MnoHelper.Version, MnoHelper.Api.Version);
            Assert.AreEqual(Environment.OSVersion.ToString() + " - " + Environment.Version.ToString(), MnoHelper.Api.LangVersion);
            ...
            Assert.IsTrue(MnoHelper.Sso.Enabled);
            Assert.IsTrue(MnoHelper.Sso.SloEnabled);
            Assert.AreEqual("https://maestrano.com", MnoHelper.Sso.Idp);
            Assert.AreEqual("http://localhost", MnoHelper.Sso.Idm);
            Assert.AreEqual("virtual", MnoHelper.Sso.CreationMode);
            Assert.AreEqual("/maestrano/auth/saml/init.aspx", MnoHelper.Sso.InitPath);
            Assert.AreEqual("/maestrano/auth/saml/consume.aspx", MnoHelper.Sso.ConsumePath);
